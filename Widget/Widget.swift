...
struct PTrackWidgetSnapshot: Codable {
    ...
    static var placeholder: PTrackWidgetSnapshot {
        ...
        return PTrackWidgetSnapshot(
            ...
            weekSummary: WeekSummary(distanceMeters: weeklyRows.reduce(0) { $0 + $1.distanceMeters }, durationSeconds: weeklyRows.reduce(0) { $0 + $1.durationSeconds }),
            ...
            annualSeries: [
                AnnualSeries(year: 2026, weeklyDistanceMeters: currentValues, weeklyDurationSeconds: currentValues.map { $0 / 4 }, visibleWeekCount: currentValues.count, totalDistanceMeters: currentValues.reduce(0, +), totalDurationSeconds: currentValues.reduce(0) { $0 + $1 / 4 }),
                AnnualSeries(year: 2025, weeklyDistanceMeters: previousValues, weeklyDurationSeconds: previousValues.map { $0 / 4 }, visibleWeekCount: previousValues.count, totalDistanceMeters: previousValues.reduce(0, +), totalDurationSeconds: previousValues.reduce(0) { $0 + $1 / 4 })
            ],
...
struct PTrackWidgetProvider: TimelineProvider {
    ...
    func getSnapshot(in context: Context, completion: @escaping (PTrackWidgetEntry) -> Void) {
        completion(PTrackWidgetEntry(date: .now, snapshot: loadSnapshot()))
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<PTrackWidgetEntry>) -> Void) {
        let now = Date()
        let entry = PTrackWidgetEntry(date: now, snapshot: loadSnapshot())
    ...
    private func loadSnapshot() -> PTrackWidgetSnapshot {
